              ...
              <tbody>
                @foreach ($notes as $item)
                  <tr>
                    ...
                    <td class="align-middle text-sm">
                      <a href="{{ route('satker.notes.action', [$item->hashed_id]) }}" class="btn btn-sm bg-gradient-info">Action
                    ...
                    <td class="align-middle text-sm">
                      <span class="badge badge-sm bg-gradient-{{ $item->status == 'open' ? 'success' : 'danger' }} btn"
                        onclick="handleView('satker','{{ $item->hashed_id }}')" data-toggle="tooltip"
                        title="Lihat Notulensi">{{ $item->status }} <div class="fa fa-eye"></div></span>
                    </td>

                    <td class="align-middle">
                      @if ($item->status != 'lock')
                        <a href="{{ route('satker.notes.edit', [$item->hashed_id]) }}"
                        ...
                        @endif
                        <a href="{{ route('satker.notes.qrcode', [$item->hashed_id]) }}" target="_blank"
                      ...
                      @endif
                      <a href="#" onclick="handleLock('satker','{{ $item->hashed_id }}')"
                      ...
                      @if ($item->status == 'lock')
                        @if($item->file_notulen == NULL)
                          <a href="{{ route('satker.export.docs', [$item->hashed_id]) }}" target="_blank" class="text-secondary font-weight-bold text-xs"
                            data-toggle="tooltip" title="Generate PDF">
                            <button class="btn btn-sm btn-danger"><i class="fa fa-file-pdf"></i></button>
                          </a>
                        @endif
                        <a href="#" onclick="handleSend('satker','{{ $item->hashed_id }}')"
                        ...
                        @else
                        <a href="#" onclick="handleDestroy('satker','{{ $item->hashed_id }}')"
